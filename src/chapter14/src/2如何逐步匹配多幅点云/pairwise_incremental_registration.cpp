...
void loadData(int argc, char **argv, std::vector<PCD, Eigen::aligned_allocator<PCD>> &models)
{
    ...
    std::vector<std::string> file_paths = {
        "src/chapter14/src/2如何逐步匹配多幅点云/pairwise/frame_00000.pcd",
        "src/chapter14/src/2如何逐步匹配多幅点云/pairwise/capture0001.pcd",
        "src/chapter14/src/2如何逐步匹配多幅点云/pairwise/capture0002.pcd",
        "src/chapter14/src/2如何逐步匹配多幅点云/pairwise/capture0003.pcd",
        "src/chapter14/src/2如何逐步匹配多幅点云/pairwise/capture0004.pcd",
        "src/chapter14/src/2如何逐步匹配多幅点云/pairwise/capture0005.pcd"
    };
    ...
    for (const auto& fname : file_paths)
    {
        ...
        std::transform(lower_fname.begin(), lower_fname.end(), lower_fname.begin(), (int (*)(int))tolower);
        //检查参数是否为一个pcd后缀的文件
        if (lower_fname.compare(lower_fname.size() - extension.size(), extension.size(), extension) == 0)
        {
            //加载点云并保存在总体的点云列表中
            PCD m;
            m.f_name = fname;
