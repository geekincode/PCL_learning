...
// 创建金字塔表面点云
void createPyramidSurface(pcl::PointCloud<pcl::PointXYZ>& cloud, const PyramidConfig& config) {
    // 生成底面四条边的点
    ...
    for(int i = 0; i < config.points_per_edge; i++) {
        ...
        // 底面四个边
        cloud.push_back(pcl::PointXYZ(-half_base + config.base_size * t, -half_base, 0));  // 前边
        cloud.push_back(pcl::PointXYZ(half_base, -half_base + config.base_size * t, 0));   // 右边
        cloud.push_back(pcl::PointXYZ(half_base - config.base_size * t, half_base, 0));    // 后边
        cloud.push_back(pcl::PointXYZ(-half_base, half_base - config.base_size * t, 0));   // 左边
    }
    ...
    for(int i = 0; i < config.points_per_edge; i++) {
        ...
        // 四个角点连线
        cloud.push_back(pcl::PointXYZ(-x, -x, z));  // 前左棱
        cloud.push_back(pcl::PointXYZ(x, -x, z));   // 前右棱
        cloud.push_back(pcl::PointXYZ(x, x, z));    // 后右棱
        cloud.push_back(pcl::PointXYZ(-x, x, z));   // 后左棱
    }
    
    // 生成顶点
    cloud.push_back(pcl::PointXYZ(0, 0, config.height));
}

// 创建实心金字塔（层叠结构）
void createSolidPyramid(pcl::PointCloud<pcl::PointXYZ>& cloud, const PyramidConfig& config) {
    float half_base = config.base_size / 2.0;
    ...
    for(int layer = 0; layer < layers; layer++) {
        ...
        for(int i = 0; i < points_this_layer; i++) {
            ...
            // 绘制本层的四条边
            cloud.push_back(pcl::PointXYZ(-half_current + current_base * t, -half_current, z));
            cloud.push_back(pcl::PointXYZ(half_current, -half_current + current_base * t, z));
            cloud.push_back(pcl::PointXYZ(half_current - current_base * t, half_current, z));
            cloud.push_back(pcl::PointXYZ(-half_current, half_current - current_base * t, z));
        }
    }
