...
parseCommandLine (int argc, char *argv[])
{
  ...
  // model_filename_ = "src/chapter13/src/1基于对应分组的三维物体识别/milk.pcd";
  // scene_filename_ = "src/chapter13/src/1基于对应分组的三维物体识别/milk_cartoon_all_small_clorox.pcd";
...
computeCloudResolution (const pcl::PointCloud<PointType>::ConstPtr &cloud)
{
  ...
  {
    if (! pcl::isFinite ((*cloud)[i]))//剔除 NAN点
    {
      continue;
    }
...
//  pcl::NormalEstimation<PointType, NormalType> norm_est;//多核 计算法线模型 OpenMP
  ...
  // 过滤模型中的无效法线和对应点
  pcl::PointCloud<PointType>::Ptr model_valid (new pcl::PointCloud<PointType>);
  pcl::PointCloud<NormalType>::Ptr model_normals_valid (new pcl::PointCloud<NormalType>);
  for (size_t i = 0; i < model->size() && i < model_normals->size(); ++i)
  {
    if (pcl::isFinite(model->at(i)) && pcl::isFinite(model_normals->at(i)))
    {
      model_valid->push_back(model->at(i));
      model_normals_valid->push_back(model_normals->at(i));
    }
  }
  *model = *model_valid;
  *model_normals = *model_normals_valid;
  std::cout << "Model valid points after normal filtering: " << model->size() << std::endl;

  norm_est.setInputCloud (scene);//场景点云
  norm_est.compute (*scene_normals);//场景点云的法线向量

  // 过滤场景中的无效法线和对应点
  pcl::PointCloud<PointType>::Ptr scene_valid (new pcl::PointCloud<PointType>);
  pcl::PointCloud<NormalType>::Ptr scene_normals_valid (new pcl::PointCloud<NormalType>);
  for (size_t i = 0; i < scene->size() && i < scene_normals->size(); ++i)
  {
    if (pcl::isFinite(scene->at(i)) && pcl::isFinite(scene_normals->at(i)))
    {
      scene_valid->push_back(scene->at(i));
      scene_normals_valid->push_back(scene_normals->at(i));
    }
  }
  *scene = *scene_valid;
  *scene_normals = *scene_normals_valid;
  std::cout << "Scene valid points after normal filtering: " << scene->size() << std::endl;
  ...
  std::cout << "Model total points: " << model->size () << "; Selected Keypoints before filtering: " << model_keypoints->size () << std::endl;
  ...
  std::cout << "Scene total points: " << scene->size () << "; Selected Keypoints before filtering: " << scene_keypoints->size () << std::endl;
  ...
  // 为模型关键点计算SHOT描述子
  pcl::PointCloud<PointType>::Ptr model_keypoints_valid (new pcl::PointCloud<PointType>);
  pcl::PointCloud<DescriptorType>::Ptr model_descriptors_valid (new pcl::PointCloud<DescriptorType>);
  descr_est.setInputCloud (model_keypoints);
  descr_est.setInputNormals (model_normals);
  descr_est.setSearchSurface (model);
  pcl::PointCloud<DescriptorType>::Ptr model_descriptors_temp (new pcl::PointCloud<DescriptorType>);
  descr_est.compute (*model_descriptors_temp);//模型点云描述子
  
  // 过滤有效的模型描述子和对应的关键点，确保一致性
  for (size_t i = 0; i < model_descriptors_temp->size(); ++i)
  {
    if (pcl::isFinite(model_descriptors_temp->at(i)) && 
        i < model_keypoints->size() && 
        pcl::isFinite(model_keypoints->at(i)))
    {
      model_descriptors_valid->push_back(model_descriptors_temp->at(i));
      model_keypoints_valid->push_back(model_keypoints->at(i));
    }
  }
  *model_keypoints = *model_keypoints_valid;
  *model_descriptors = *model_descriptors_valid;
  std::cout << "Model valid keypoints: " << model_keypoints->size() << "; Valid descriptors: " << model_descriptors->size() << std::endl;

  // 为场景关键点计算SHOT描述子
  pcl::PointCloud<PointType>::Ptr scene_keypoints_valid (new pcl::PointCloud<PointType>);
  pcl::PointCloud<DescriptorType>::Ptr scene_descriptors_valid (new pcl::PointCloud<DescriptorType>);
  descr_est.setInputCloud (scene_keypoints);
  descr_est.setInputNormals (scene_normals);
  descr_est.setSearchSurface (scene);
  pcl::PointCloud<DescriptorType>::Ptr scene_descriptors_temp (new pcl::PointCloud<DescriptorType>);
  descr_est.compute (*scene_descriptors_temp);//场景点云描述子
  
  // 过滤有效的场景描述子和对应的关键点，确保一致性
  for (size_t i = 0; i < scene_descriptors_temp->size(); ++i)
  {
    if (pcl::isFinite(scene_descriptors_temp->at(i)) && 
        i < scene_keypoints->size() && 
        pcl::isFinite(scene_keypoints->at(i)))
    {
      scene_descriptors_valid->push_back(scene_descriptors_temp->at(i));
      scene_keypoints_valid->push_back(scene_keypoints->at(i));
    }
  }
  *scene_keypoints = *scene_keypoints_valid;
  *scene_descriptors = *scene_descriptors_valid;
  std::cout << "Scene valid keypoints: " << scene_keypoints->size() << "; Valid descriptors: " << scene_descriptors->size() << std::endl;
