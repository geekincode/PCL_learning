...
void csv2pointCloud(std::string filename, pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud)
{
    ...
    while (getline(fin, line))   //整行读取，换行符“\n”区分，遇到文件尾标志eof终止读取
    {
        ...
        p.intensity = atof(Trim(fields[7]).c_str());
...
int main()
{
    ...
    std::string filename = "./src/chapter01/pcd/car_mid360_2.csv";
    csv2pointCloud(filename, cloud);
    cloud->width = (int)cloud->points.size();
    cloud->height = 1;
    pcl::io::savePCDFileASCII("./src/chapter01/output/car_mid360_2.pcd", *cloud);
