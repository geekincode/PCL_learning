...
#include <pcl/visualization/pcl_visualizer.h>  // 修正为正确的头文件
#include <pcl/search/kdtree.h>                 // 显式包含KDTree头文件
#include <pcl/point_cloud.h>                   // 添加缺失的关键头文件
#include <pcl/common/common.h>                 // 添加缺失的关键头文件

int main (){
     //打开点云代码
     pcl::PointCloud<pcl::PointXYZ>::Ptr cloud (new pcl::PointCloud<pcl::PointXYZ>);
     pcl::io::loadPCDFile ("./src/chapter10/src/1估计整个点云表面法向量/table_scene_lms400.pcd", *cloud);
     ...
     writer.write<pcl::Normal> ( "./src/chapter10/src/1估计整个点云表面法向量/table_cloud_normals.pcd" , *cloud_normals, false); // 保存文件
     //可视化
